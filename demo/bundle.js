...
/***/ function(module, exports) {
	...
	var ColorPicker, extendOptions,
	  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
	  slice = [].slice;

	ColorPicker = (function() {
	  ColorPicker.prototype.template = '<div class="small-colorpicker">' + '<div class="small-colorpicker-colors"></div>' + '</div>';

	  ColorPicker.prototype.colors = ['ee9999', 'fdd4a3', 'ffee99', 'dfefc5', 'c1e39d', 'bddcdd', '99c7ec', 'c5dfe9', 'c2b7c8', 'ddbec7', 'f7c6c3', 'f1e2cf', 'cccccc', 'e24d4d', 'fbb35f', 'ffe24d', 'c8e29a', '93cd54', '8cc1c3', '4d9ddd', '9ac8d9', '95829e', 'c38d9e', 'f09c95', 'e7cdab', 'a5a5a5', 'd50000', 'f9931a', 'ffd500', 'b0d66e', '64b80a', '5ba7a9', '0173cf', '6fb0c8', '674c75', 'a95c74', 'ea7168', 'dcb787', '7f7f7f', 'aa0000', 'c77615', 'ccaa00', '8dab58', '509308', '498687', '015ca6', '598da0', '523d5e', '874a5d', 'bb5a53', 'b0926c', '666666', '800000', '955810', '998000', '6a8042', '3c6e06', '376465', '01457c', '436a78', '3e2e46', '653746', '8c443e', '846e51', '4c4c4c'];

	  function ColorPicker(element, options) {
	    this.options = options;
	    this.closePicker = bind(this.closePicker, this);
	    this.clickClosePicker = bind(this.clickClosePicker, this);
	    if (!this.options) {
	      this.options = {};
	    }
	    ...
	    this.element.on('click', (function(_this) {
	      return function(event) {
	        if (_this.options.colorAttribute != null) {
	          _this.colors = $(element).attr(_this.options.colorAttribute).split(",");
	        }
	        _this.picker || (_this.picker = _this.createColorPicker());
	        _this.placePicker();
	        _this.picker.off("click.colorPickerColor");
	        _this.picker.on("click.colorPickerColor", ".small-colorpicker-color", function(event) {
	          var hex;
	          hex = $(event.target).data('color');
	          _this.input.minicolors('value', hex);
	          _this.setHexColor(hex);
	          _this.closePicker();
	          return false;
	        });
	        return $(window).on('click', _this.clickClosePicker);
	      };
	    })(this));
	  }

	  ColorPicker.prototype.clickClosePicker = function(event) {
	    if ($(event.target)[0] === this.element[0]) {
	      return;
	    }
	    if ($(event.target).is('.small-colorpicker-custom')) {
	      return;
	    }
	    if ($(event.target).is('.minicolors')) {
	      return;
	    }
	    if ($(event.target).parents('.minicolors').length > 0) {
	      return;
	    }
	  ...
	  ColorPicker.prototype.closePicker = function(event) {
	    if (window.enableScrolling) {
	      enableScrolling();
	    }
	    $(window).off('click', this.clickClosePicker);
	    if (this.hexColor && this.color) {
	      this.element.trigger({
	        type: 'changeColor',
	        color: {
	          hex: this.hexColor,
	          converted: this.color
	        }
	      });
	    }
	    if (this.picker) {
	  ...
	  ColorPicker.prototype.createColorPicker = function() {
	    var color, i, initialColor, len, picker, ref;
	    picker = $(this.template).appendTo("body");
	    picker.data('colorPicker', this);
	    if (this.options.displayRight) {
	    ...
	    initialColor = this.element.data('color') || '#FFFFFF';
	    if (initialColor[0] !== '#') {
	    ...
	    this.input = picker.find('.small-colorpicker-custom');
	    this.input.minicolors({
	      theme: 'colorpicker',
	      change: (function(_this) {
	        return function(hex) {
	          return _this.setHexColor(hex);
	        };
	      })(this)
	    });
	    this.input.css({
	      backgroundColor: initialColor,
	      color: this.fontFromColor(initialColor)
	    });
	    ...
	    picker.find('.minicolors-panel > div').each(function(_, div) {
	      return $(div).css({
	        top: parseInt($(div).css('top')) + 3,
	        left: parseInt($(div).css('left')) + 3
	      });
	    });
	    return picker.find('.small-colorpicker-colors').append("<div class='clearfix'></div>").end();
	  };

	  ColorPicker.prototype.setHexColor = function(hex) {
	    var rgb;
	    if (!/(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/i.test(hex)) {
	      return;
	    }
	    if (!this.options.dontSetElementColor) {
	      this.element.css({
	        backgroundColor: hex
	      });
	    }
	    this.input.css({
	      backgroundColor: hex,
	      color: this.fontFromColor(hex)
	    });
	    hex = hex.replace('#', '');
	    rgb = this.hexToRGB(hex);
	    this.hexColor = hex;
	    return this.color = (1 << 24) | (parseInt(rgb.r) << 16) | (parseInt(rgb.g) << 8) | parseInt(rgb.b);
	  };

	  ColorPicker.prototype.placePicker = function() {
	    var css, elemRect;
	    this.picker.show();
	    if (window.preventScrolling) {
	      preventScrolling();
	    }
	    elemRect = this.element[0].getBoundingClientRect();
	    css = {};
	    if (elemRect.bottom - elemRect.height / 2 < window.innerHeight / 2) {
	      css.top = this.element.offset().top + this.element.outerHeight();
	      this.picker.addClass('small-colorpicker-top-arrow').removeClass('small-colorpicker-bottom-arrow');
	    } else {
	      css.top = this.element.offset().top - this.picker.outerHeight();
	      this.picker.addClass('small-colorpicker-bottom-arrow').removeClass('small-colorpicker-top-arrow');
	    }
	    if (elemRect.right - elemRect.width / 2 < window.innerWidth / 2) {
	      css.left = this.element.offset().left - 8;
	      this.picker.addClass('small-colorpicker-left-arrow').removeClass('small-colorpicker-right-arrow');
	    } else {
	      css.left = this.element.offset().left + this.element.width() - this.picker.outerWidth() + 8;
	      this.picker.addClass('small-colorpicker-right-arrow').removeClass('small-colorpicker-left-arrow');
	    }
	    return this.picker.css(css);
	  };

	  ColorPicker.prototype.fontFromColor = function(hex) {
	    var a, rgb;
	    rgb = this.hexToRGB(hex.replace('#', ''));
	  ...
	  ColorPicker.prototype.hexToRGB = function(colorHex) {
	    var bigint;
	...
	extendOptions["colorPicker"] = function() {
	  var args, option;
	  option = arguments[0], args = 2 <= arguments.length ? slice.call(arguments, 1) : [];
	  return this.each(function() {
	    var $this, data;
	    $this = $(this);
	    if (!(data = $this.data("colorPicker"))) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	if(false) {
		...
		if(!content.locals) {
			module.hot.accept("!!./../../../../node_modules/css-loader/index.js!./../../../../node_modules/less-loader/index.js!./color_picker.css.less", function() {
				var newContent = require("!!./../../../../node_modules/css-loader/index.js!./../../../../node_modules/less-loader/index.js!./color_picker.css.less");
